...
export default function QuestionsList() {
  ...
  if (activeTags.length > 0) {
    filtered = filtered.filter((q) =>
      q.questiontags?.some((tg: string) =>
        activeTags.includes(tg.toLowerCase())
      )
    );
  }
  ...
  if (activeMinAnswers > 0) {
    filtered = filtered.filter(
      (q) => (q.noofanswer || 0) >= activeMinAnswers
    );
  }
  ...
  filtered = [...filtered].sort((a, b) => {
    if (activeSort === "oldest")
      return (
        new Date(a.askedon).getTime() - new Date(b.askedon).getTime()
      );

    if (activeSort === "mostVoted")
      return (b.upvote?.length || 0) - (a.upvote?.length || 0);

    if (activeSort === "mostAnswered")
      return (b.noofanswer || 0) - (a.noofanswer || 0);
    ...
    <Mainlayout>
      ...
      {hasActiveFilter && (
        <div className="text-xs text-gray-500 mb-3">
          {t("questions.showing")} {filtered.length} {t("questions.of")}{" "}
          {questions.length} {t("questions.allQuestions").toLowerCase()}
          {activeTags.length > 0 && (
          ...
          {filtered.map((q) => (
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-1">
                {q.upvote?.length || 0} {t("questions.upvotes")} ·{" "}
                {q.noofanswer || 0} {t("questions.answers")} ·{" "}
                {t("questions.asked")}{" "}
